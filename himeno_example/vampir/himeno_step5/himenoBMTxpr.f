...
C
      if(id.eq.23)then
        print *,'in buffers',sum(pack_p11),sum(pack_p21),
     &                 sum(pack_p31)
        print *,'in buffers',sum(pack_p12),sum(pack_p22),
     &                 sum(pack_p32)
      endif
     ...
     & unpack_p21,unpack_p22,unpack_p31,unpack_p32)
      if(id.eq.23)then
        print *,'out buffers',sum(unpack_p11),sum(unpack_p21),
     &                 sum(unpack_p31)
        print *,'out buffers',sum(unpack_p12),sum(unpack_p22),
     &                 sum(unpack_p32)
      endif
c
#ifdef GPU
...
!$OMP&   shared  (p,wrk2)    
         ...
         DO K=1,kmax
            DO J=1,jmax
                ii = j + (k-1)*jmax
                  p(1,j,k)=unpack_p11(ii)
                  p(imax,j,k)=unpack_p12(ii)
            enddo
         enddo
#ifdef GPU
!$ACC  end parallel loop 
...
!$OMP&   private (i,jj,k)                                    
         ...
         DO K=1,kmax
            DO I=1,imax
                jj = i + (k-1)*imax
                  p(i,1,k)=unpack_p21(jj)
                  p(i,jmax,k)=unpack_p22(jj)
            enddo
         enddo
#ifdef GPU
!$ACC  end parallel loop 
...
!$OMP&   private (i,j,kk)                                    
         ...
         DO J=1,jmax
            DO I=1,imax
                kk = i +(j-1)*imax 
                  p(i,j,1)=unpack_p31(kk)
                  p(i,j,kmax)=unpack_p32(kk)
            enddo
         enddo
#ifdef GPU
!$ACC  end parallel loop 
