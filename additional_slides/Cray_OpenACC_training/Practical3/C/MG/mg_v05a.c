...
#pragma acc parallel loop vector_length(NTHREADS) private(u1,u2) \
  copyin(u[0:n1*n2*n3],v[0:n1*n2*n3],a[0:4]) \
...
#pragma acc parallel loop vector_length(NTHREADS) private(x1,y1)	\
  copyout(s[0:m1j*m2j*m3j])			\
  copyin(r[0:m1k*m2k*m3k])
...
#pragma omp for private(i123)
#pragma acc parallel loop vector_length(NTHREADS) \
  private(tmp,i123) reduction(+:p_s) reduction(max:p_a) copyin(r[0:n3*n2*n1])
    for (i3 = 1; i3 < n3-1; i3++) {
	for (i2 = 1; i2 < n2-1; i2++) {
            for (i1 = 1; i1 < n1-1; i1++) {
	      i123 = i1 + n1*(i2 + n2*i3);
...
#pragma acc data copy(u[0:n3*n2*n1])
    {
    /* axis = 1 */
#pragma omp for
#pragma acc parallel loop vector_length(NTHREADS)
...
#pragma acc parallel copy(z[0:n3*n2*n1]) copyin(jg)
{
#pragma acc loop
    for (i = MM-1; i >= m0; i--) {
