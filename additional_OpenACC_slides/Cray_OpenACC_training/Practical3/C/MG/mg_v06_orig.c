...
#pragma acc data copyin(a[0:4],c[0:4])
    {
...
#ifdef _OPENACC
    //****************************************************************
    /* Now manually deep-create these arrays on the GPU using the Cray extended
     * runtime API
     */
    double ****acc_u = (double ****)cray_acc_create(u,(lt+1)*sizeof(double ***));
    for (l=lt; l >=1; l--) {
      double ***acc_ul = (double ***)cray_acc_create(u[l],m3[l]*sizeof(double **));
      SET_ACC_PTR(acc_u[l], acc_ul);
      for (k=0; k<m3[l]; k++) {
	double **acc_ulk = (double **)cray_acc_create(u[l][k],m2[l]*sizeof(double *));
	SET_ACC_PTR(acc_ul[k], acc_ulk);
	for (j=0; j<m2[l]; j++) {
	  double *acc_ulkj = (double *)cray_acc_create(u[l][k][j],m1[l]*sizeof(double));
	  SET_ACC_PTR(acc_ulk[j],acc_ulkj);
	}
      }
    }

  double ***acc_v = (double ***)cray_acc_create(v,m3[lt]*sizeof(double **));
  for (k=0; k<m3[lt]; k++) {
    double **acc_vk = (double **)cray_acc_create(v[k],m2[lt]*sizeof(double *));
    SET_ACC_PTR(acc_v[k], acc_vk);
    for (j=0; j<m2[lt]; j++) {
      double *acc_vkj = (double *)cray_acc_create(v[k][j],m1[lt]*sizeof(double));
      SET_ACC_PTR(acc_vk[j],acc_vkj);
    }
  }

    double ****acc_r = (double ****)cray_acc_create(r,(lt+1)*sizeof(double ***));
    for (l=lt; l >=1; l--) {
      double ***acc_rl = (double ***)cray_acc_create(r[l],m3[l]*sizeof(double **));
      SET_ACC_PTR(acc_r[l], acc_rl);
      for (k=0; k<m3[l]; k++) {
	double **acc_rlk = (double **)cray_acc_create(r[l][k],m2[l]*sizeof(double *));
	SET_ACC_PTR(acc_rl[k], acc_rlk);
	for (j=0; j<m2[l]; j++) {
	  double *acc_rlkj = (double *)cray_acc_create(r[l][k][j],m1[l]*sizeof(double));
	  SET_ACC_PTR(acc_rlk[j],acc_rlkj);
	}
      }
    }
...
#ifdef _OPENACC
    //****************************************************************
    /* Now manually deep-delete these arrays on the GPU using the Cray extended
     * runtime API (just for good housekeeping)
     */
    for (l=lt; l>=1; l--) {
      for (k=0; k<m3[l]; k++) {
	for (j=0; j<m2[l]; j++) 
	  cray_acc_delete((void *) u[l][k][j]);
	cray_acc_delete((void *) u[l][k]);
      }
      cray_acc_delete((void *) u[l]);
    }
    cray_acc_delete((void *) u);

    for (k=0; k<m3[lt]; k++) {
      for (j=0; j<m2[lt]; j++) 
	cray_acc_delete((void *) v[k][j]);
      cray_acc_delete((void *) v[k]);
    }
    cray_acc_delete((void *) v);
    
    for (l=lt; l>=1; l--) {
      for (k=0; k<m3[l]; k++) {
	for (j=0; j<m2[l]; j++) 
	  cray_acc_delete((void *) r[l][k][j]);
	cray_acc_delete((void *) r[l][k]);
      }
      cray_acc_delete((void *) r[l]);
    }
    cray_acc_delete((void *) r);

    //****************************************************************
#endif /* _OPENACC */


    } // acc end data // copyin(a[0:4],c[0:4])
...
#pragma acc parallel loop vector_length(NTHREADS) private(u1,u2) \
  present(u[0:n3],v[0:n3],a[0:4],r[0:n3])
    for (i3 = 1; i3 < n3-1; i3++) {
	for (i2 = 1; i2 < n2-1; i2++) {
            for (i1 = 0; i1 < n1; i1++) {
...
#pragma omp for    
#pragma acc parallel loop vector_length(NTHREADS) \
  private(tmp) reduction(+:p_s) reduction(max:p_a) present(r[0:n3])
    for (i3 = 1; i3 < n3-1; i3++) {
	for (i2 = 1; i2 < n2-1; i2++) {
            for (i1 = 1; i1 < n1-1; i1++) {
...
c-------------------------------------------------------------------*/
    ...
    /* axis = 1 */
#pragma omp for
#pragma acc parallel loop vector_length(NTHREADS) present(u[0:n3])
    for ( i3 = 1; i3 < n3-1; i3++) {
#pragma acc loop independent
	for ( i2 = 1; i2 < n2-1; i2++) {
...
#pragma acc parallel present(z[0:n3]) copyin(jg)
    {
#pragma acc loop
    for (i = MM-1; i >= m0; i--) {
...
#ifdef _OPENACC
    //****************************************************************
    // This does a deep update host of the data
    cray_acc_update_host(z,n3*sizeof(double **));
    for (int k=0; k<n3; k++) {
      z[k] = (double **)(cray_acc_hostptr((void *)z[k]));
      cray_acc_update_host(z[k],n2*sizeof(double *));
      for (int j=0; j<n2; j++) {
	z[k][j] = (double *)(cray_acc_hostptr((void *)z[k][j]));
	cray_acc_update_host(z[k][j],n1*sizeof(double));
      }
    }
    //****************************************************************
#endif /* _OPENACC */
...
c-------------------------------------------------------------------*/
    ...
    int i1, i2, i3;
#pragma omp for    
#pragma acc parallel loop vector_length(NTHREADS) present(z[0:n3])
    for (i3 = 0;i3 < n3; i3++) {
	for (i2 = 0; i2 < n2; i2++) {
            for (i1 = 0; i1 < n1; i1++) {
		z[i3][i2][i1] = 0.0;
