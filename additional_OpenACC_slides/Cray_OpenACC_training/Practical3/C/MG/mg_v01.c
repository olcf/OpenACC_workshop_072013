...
#ifdef CRAYPAT
// We want to use the CrayPAT API
#include "pat_api.h"
#endif
...
#ifdef CRAYPAT
    // Switch CrayPAT collection off initially
...
#pragma omp single
    ...
    timer_stop(T_INIT);
#ifdef CRAYPAT
    // Now switch CrayPAT collection back on
    istat = PAT_record(PAT_STATE_ON);
#endif
    timer_start(T_BENCH);
...
#pragma omp master
    ...
    timer_stop(T_BENCH);
#ifdef CRAYPAT
    // Switch CrayPAT collection off again
    istat = PAT_record(PAT_STATE_OFF);
#endif
