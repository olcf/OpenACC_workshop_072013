...
/*      Define this to use the original code with the private temporary 
c       array.  
c       Undefine it to use the version with the temporary arrays inlined.*/
#undef Resid_original
...
c-------------------------------------------------------------------*/
		...
		u[i123-n1-n12] + u[i123+n1-n12]
		       + u[i123-n1+n12] + u[i123+n1+n12] +
		//u1[i1-1] + 
		u[i123-n1-1] + u[i123+n1-1]
		       + u[i123-n12-1] + u[i123+n12-1] +
		//u1[i1+1] )
	        u[i123-n1+1] + u[i123+n1+1]
		  + u[i123-n12+1] + u[i123+n12+1] )
		      - a[3] * ( 
				//u2[i1-1] + 
		u[i123-n1-n12-1] + u[i123+n1-n12-1]
		       + u[i123-n1+n12-1] + u[i123+n1+n12-1] +
		//u2[i1+1] );
		u[i123-n1-n12+1] + u[i123+n1-n12+1]
		+ u[i123-n1+n12+1] + u[i123+n1+n12+1] );
...
#pragma omp for private(i123)
    // This is the big problem. If I can get this working, all
    // other kernels are fine and I can introduce the data region.
#pragma acc parallel loop vector_length(NTHREADS) \
  private(tmp,i123) reduction(+:p_s) reduction(max:p_a) present(r[0:n3*n2*n1])
    for (i3 = 1; i3 < n3-1; i3++) {
	for (i2 = 1; i2 < n2-1; i2++) {
            for (i1 = 1; i1 < n1-1; i1++) {
	      i123 = i1 + n1*(i2 + n2*i3);
	      p_s = p_s + r[i123] * r[i123];
	      tmp = fabs(r[i123]);
	      if (tmp > p_a) p_a = tmp;
	    }
	}
...
c-------------------------------------------------------------------*/
    ...
    /* axis = 1 */
#pragma omp for
#pragma acc parallel loop gang vector_length(NTHREADS) present(u[0:n3*n2*n1])
    for ( i3 = 1; i3 < n3-1; i3++) {
#pragma acc loop independent // Needed for cce/8.2.0.141
	for ( i2 = 1; i2 < n2-1; i2++) {
	  int i23 = n1*(i2+n2*i3);
    ...
    /* axis = 2 */
#pragma omp for
#pragma acc parallel loop vector_length(NTHREADS) present(u[0:n3*n2*n1])
    for ( i3 = 1; i3 < n3-1; i3++) {
#pragma acc loop independent // Needed for cce/8.2.0.141
	for ( i1 = 0; i1 < n1; i1++) {
	  int i13 = i1 + n12*i3;
	  u[n1*(n2-1)+i13] = u[n2*1+i13];
	  u[n1*0+i13] = u[n1*(n2-2)+i13];
	}
    }
...
#pragma acc loop independent // Needed for cce/8.2.0.141
	for ( i1 = 0; i1 < n1; i1++) {
	  int i12 = i1 + n1*i2;
	  u[n12*(n3-1)+i12] = u[n12*1+i12];
	  u[n12*0+i12] = u[n12*(n3-2)+i12];
	}
    }
}
...
#ifdef _OPENACC
    // As we are not sure if this array is in a data region (v)
    // or not (u,r), we do the update manually
    cray_acc_update_host(z,n3*n2*n1*sizeof(double **));
    /* Otherwise we could use the update directive:
     *  #pragma acc data present(z[0:n3*n2*n1])
     *  {
     *  #pragma acc update host(z[0:n3*n2*n1])
     *  }
    */
#endif
